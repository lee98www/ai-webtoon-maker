...
## [2026-01-13] 시트 생성 진행 상태 프로그레스 UI 추가

### 문제
- 콘티 및 시트 생성 중 사용자에게 진행 상태가 표시되지 않음
- 생성 중인지, 오류인지 구분 불가

### 추가

**`BlueprintStep.tsx`**:
- `GenerationProgress` 컴포넌트 추가: 3단계 진행 상태 시각화
  - 콘티 생성 → 캐릭터 시트 → 장소 시트
- 각 단계별 상태 표시:
  - 대기 (회색)
  - 진행 중 (파란색 + 스피너)
  - 완료 (녹색 + 체크)
  - 실패 (빨간색 + X)
- `generationStep` state 추가: 현재 단계 추적
- `sheetErrors` state 추가: 에러 메시지 수집 및 표시
- 에러 발생 시 상세 메시지 표시

### 관련 파일
- `src/components/steps/storyboard/BlueprintStep.tsx` (라인 175-274)

...
export const RenderStep: React.FC = () => {
  const { project, isProcessing, markStepComplete, setStep } = useProjectStore();
        ...
        {allCompleted && (
          <div className="px-6 py-4 border-t border-slate-200 bg-white">
            <button
              ...
              className="w-full py-3 bg-emerald-600 text-white rounded-lg font-medium text-sm hover:bg-emerald-700 transition flex items-center justify-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
              </svg>
              <span>웹툰 뷰어에서 보기</span>
            </button>
            <p className="text-xs text-slate-400 text-center mt-2">
              뷰어에서 다운로드 가능 (ZIP / 합본 이미지)
            </p>
          </div>
        )}
      </div>
